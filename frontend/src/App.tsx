...
const Page0 = lazy(() => import('./pages/story-7/index'));
const Page1 = lazy(() => import('./pages/story-8/index'));
  ...
  return (
    <BrowserRouter>
      <div style={{ fontFamily: 'system-ui, sans-serif', minHeight: '100vh', background: '#f9fafb' }}>
        <nav style={NAV_STYLE}>
          <Link to="/" style={ACTIVE_STYLE}>Orchestrator Dashboard</Link>
          <Link to="/document-processor" style={LINK_STYLE}>Document Processor</Link>
        </nav>
        <main style={{ padding: '2rem 1.5rem', maxWidth: 1024, margin: '0 auto' }}>
          <Suspense fallback={<p>Loading…</p>}>
            <Routes>
              <Route path="/" element={<Page0 />} />
              <Route path="/document-processor" element={<Page1 />} />
